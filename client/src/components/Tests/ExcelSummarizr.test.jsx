...
import ExcelSummarizr from "../ExcelSummarizr";
...
describe("ExcelSummarizr", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  test("renders file uploader and summarize button", () => {
    render(<ExcelSummarizr triggerScroll={() => {}} />);
    expect(
      screen.getByText(/Click or drag a .xlsx file here to upload/i)
    ).toBeInTheDocument();
    expect(
  ...
  test("shows error for invalid file type", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Please provide an Excel \(.*\.xlsx.*\) file!/i)
      ).toBeInTheDocument();
    });
  });
  ...
  test("shows error for file too large", async () => {
    render(<ExcelSummarizr triggerScroll={() => {}} />);
    const fileInput = screen.getByTestId("file-input");
    const largeFile = new File(["a".repeat(6 * 1024 * 1024)], "big.xlsx", {
  ...
  test("enables summarize button for valid file", async () => {
    render(<ExcelSummarizr triggerScroll={() => {}} />);
    const fileInput = screen.getByTestId("file-input");
    const validFile = new File(["dummy"], "test.xlsx", {
      type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });
        ...
        new Promise((resolve) =>
          setTimeout(
            () =>
              resolve({
                ok: true,
                json: () => Promise.resolve({ summary: "Excel summary here." }),
              }),
            100
          )
        )
  ...
  test("displays summary and allows copying", async () => {
    ...
    const button = screen.getByRole("button", { name: /Summarize/i });
    fireEvent.click(button);
    expect(await screen.findByText("Excel summary here.")).toBeInTheDocument();
