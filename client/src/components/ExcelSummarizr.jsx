...
export default function ExcelSummarizr({ summaryRef, triggerScroll }) {
  ...
  const handleFileUpload = (fileObj) => {
    ...
    // Validate type
    if (
      (!fileObj.type || fileObj.type !== "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") &&
      !fileObj.name.toLowerCase().endsWith(".xlsx")
    ) {
      setError("Please provide an Excel (.xlsx) file!");
      triggerScroll(true);
      return;
    }
    // Validate size
  ...
  const handleSummarize = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/summarize/excel", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="w-full max-w-6xl bg-[#111827]/95 rounded-3xl shadow-2xl p-10 flex flex-col items-center border border-cyan-200/25 ring-2 ring-cyan-400/10 backdrop-blur-xl transition-all hover:scale-[1.01] hover:shadow-[0_4px_60px_0_rgba(0,255,255,0.25)]">
      ...
      <FileUploader
        ...
        label="Click or drag a .xlsx file here to upload"
        helperText="Only .xlsx files are supported for Excel summarization (max 5MB)"
